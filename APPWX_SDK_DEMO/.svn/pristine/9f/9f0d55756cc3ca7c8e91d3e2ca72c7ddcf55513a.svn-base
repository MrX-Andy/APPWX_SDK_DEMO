...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> parent, View covertView, int position,long id) {
	...
	String mStrDemos[] = {
            "侧滑菜单(silder menu)",
            "app消息提醒(app message)",
            "google齿轮控件(google wheel)",
            "圆盘菜单(circle menu)",
            "listview分组title浮动(staellitemview)",
            "淡入淡出ActionBar(fade ActionBae)",
            "径向菜单(Radial Menu)",
            "Path菜单(Satellite Menu)",
            "弧形和纵向回弹菜单(Arc and Ray)",
            "自定义imageview(图片缓存)",
            "瀑布流(PLA -PinterestLikeAdapterView)",
            "listview下拉刷新(pull down refersh listview)",
            "ViewPager动画特效(3D JazzyViewPager)",
            "Activity动画(ActivityAnimator)",
            "抽屉效果(SlidingUpPanel)"
    };
	
	Class<?> mActivities[] = {
			MenuDrawerActivity.class,
			AppMsgActivity.class,
			WheelActivity.class,
			WheelMenuActivity.class,
			StickyListViewActivity.class,
			HomeActivity.class,
			MainMenu.class,
			SatelliteMenuActivity.class,
			ArcAndRayActivity.class,
			SmartImageViewActivity.class,
			WaterFallActivity.class,
			PullRefreshListview.class,
			JazzyViewPagerActivity.class,
			ActivityAnimatorActivity.class,
