...
public class MenuDrawerActivity extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mAdapter.addSample("ListActivity sample", "演示如何使用抽屉与ListActivity的.",
				ListActivitySample.class);
		mAdapter.addSample("Window sample", "全屏滑动.", WindowSample.class);
		mAdapter.addSample("ViewPager", "只能拖开时ViewPager是在第一页上的一个左抽屉",
				ViewPagerSample.class);
		mAdapter.addSample("Layout xml", "在XML中定义的抽屉，其菜单和内容",
				LayoutSample.class);
		mAdapter.addSample("Fragments", "Fragments使用的内容片段",
				FragmentSample.class);
		mAdapter.addSample("ActionBarSherlock sample",
				"使用ActionBarSherlock展示柜的抽屉.", ActionBarSherlockSample.class);
		...
		mAdapter.addSample("Left overlay", "抽屉可以从左边拖动.",
				LeftOverlaySample.class);
		mAdapter.addSample("Top overlay", "抽屉可以从顶部拖动.", TopOverlaySample.class);
		mAdapter.addSample("Right overlay", "抽屉可以从右边拖.",
				RightOverlaySample.class);
		mAdapter.addSample("Bottom overlay", "抽屉可以从底部边拖.",
	...
	public class MenuDrawerAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (getItemViewType(position) == 0) {
				...
				if (v == null) {
					v = (TextView) getLayoutInflater().inflate(
							R.layout.list_row_sample_header, parent, false);
				}
